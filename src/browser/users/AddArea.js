...
export default class Invite extends Component {
  ...
  render() {
    ...
    const buttons = [
      ...
      (stepIndex === 1) && <FlatButton type="submit" label={msg('user.addArea')} onClick={() => submit('filter')} />,
    ];

    return (
      <Dialog
        title={msg('user.addArea')}
        actions={buttons}
        open={Boolean(true)}
      ...
      >
        <p>{msg('user.geo.description')}</p>
        <Stepper activeStep={stepIndex}>
          <Step>
            <StepLabel>{msg('geo.selectArea.header')}</StepLabel>
          </Step>
          <Step>
            <StepLabel>{msg('user.geo.notificationFrequency')}</StepLabel>
          </Step>
        </Stepper>
        {(stepIndex === 0) ?
          <TrashFilter
